...
def test_initial_liquidity(w3, contract, DAI_token, assert_fail):
    ...
    contract.__callback(QUERY_ID, '1000000001', transact={'from': oraclize_owner})
...
def test_liquidity_pool(w3, contract, DAI_token, USDC_token, assert_fail):
    ...
    contract.__callback(QUERY_ID, str(DAI_ADDED), transact={'from': oraclize_owner})

    USDC_token.transfer(user2, 15*10**18, transact={})
    USDC_token.approve(contract.address, 15*10**18, transact={'from': user2})
    USDC_ADDED = 3*10**9
    ...
    assert contract.balanceOf(user2) == USDC_ADDED
    ...
    assert contract.balanceOf(user2) == USDC_ADDED - TRANSFERRED_AMOUNT
    ...
    contract.removeLiquidity(USDC_token.address, USDC_ADDED - TRANSFERRED_AMOUNT + 1, DEADLINE, transact={'from': user2})
    assert_fail(lambda: contract.__callback(QUERY_ID, str(USDC_ADDED - TRANSFERRED_AMOUNT + 1), transact={'from': oraclize_owner}))
    # deadline < block.timestamp
    ...
    assert_fail(lambda: contract.__callback(QUERY_ID, str(DAI_ADDED + 1), DEADLINE, transact={'from': oraclize_owner}))
    ...
    contract.__callback(QUERY_ID, str(DAI_ADDED), transact={'from': oraclize_owner})
    contract.removeLiquidity(USDC_token.address, USDC_ADDED - TRANSFERRED_AMOUNT - DAI_ADDED, DEADLINE, transact={'from': user2})
    contract.__callback(QUERY_ID, str(USDC_ADDED - TRANSFERRED_AMOUNT - DAI_ADDED), transact={'from': oraclize_owner})
    assert contract.poolOwnership(user2) == 0
    assert contract.poolOwnership(user1) == 1
    contract.removeLiquidity(USDC_token.address, DAI_ADDED + TRANSFERRED_AMOUNT, DEADLINE, transact={'from': user1})
    contract.__callback(QUERY_ID, str(DAI_ADDED + TRANSFERRED_AMOUNT), transact={'from': oraclize_owner})
    assert contract.totalSupply() == 0
    assert contract.balanceOf(user1) == 0
    assert contract.balanceOf(user2) == 0
    assert USDC_token.balanceOf(user1) == DAI_ADDED + TRANSFERRED_AMOUNT
