...
def test_initial_liquidity(w3, contract, DAI_token, assert_fail):
    ...
    assert_fail(lambda: contract.addLiquidity('DAI'.encode(), DEADLINE, transact={'value': 999999999, 'from': a1}))
    DAI_ADDED = 10**9
    contract.addLiquidity('DAI'.encode(), DEADLINE, transact={'value': DAI_ADDED, 'from': a1})
...
def test_liquidity_pool(w3, contract, DAI_token, USDC_token, assert_fail):
    ...
    USDC_ADDED = 3*10**9
    contract.addLiquidity('USDC'.encode(), DEADLINE, transact={'value': USDC_ADDED, 'from': a2})
    ...
    assert_fail(lambda: contract.addLiquidity('DAI'.encode(), 1, transact={'value': DAI_ADDED, 'from': a1}))
    ...
    assert_fail(lambda: contract.removeLiquidity('USDC'.encode(), USDC_ADDED - TRANSFERRED_AMOUNT + 1, DEADLINE, transact={'from': a2}))
    # deadline < block.timestamp
    assert_fail(lambda: contract.removeLiquidity('USDC'.encode(), TRANSFERRED_AMOUNT, 1, transact={'from': a2}))
    # amount > token liquidity
    assert_fail(lambda: contract.removeLiquidity('DAI'.encode(), DAI_ADDED + 1, DEADLINE, transact={'from': a2}))

    # First and second liquidity providers remove their remaining liquidity
    contract.removeLiquidity('DAI'.encode(), DAI_ADDED, DEADLINE, transact={'from': a2})
    contract.removeLiquidity('USDC'.encode(), USDC_ADDED - TRANSFERRED_AMOUNT - DAI_ADDED, DEADLINE, transact={'from': a2})
    assert contract.ownership(a2) == 0
    assert contract.ownership(a1) == 1
    contract.removeLiquidity('USDC'.encode(), DAI_ADDED + TRANSFERRED_AMOUNT, DEADLINE, transact={'from': a1})
    assert contract.totalSupply() == 0
    assert contract.balanceOf(a1) == 0
