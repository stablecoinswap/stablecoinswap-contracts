...
def test_liquidity_gusd(w3, contract, DAI_token, GUSD_token, price_oracle, assert_fail):
    owner = w3.eth.accounts[0]
    user1 = w3.eth.accounts[1]
    user2 = w3.eth.accounts[2]
    TOKEN_PRICE = 1.01
    ...
    price_oracle.updatePrice(GUSD_token.address, INT_TOKEN_PRICE * 10**(18-2), transact={'from': owner})
    ...
    # amount > token liquidity
    assert_fail(lambda: contract.removeLiquidity(GUSD_token.address, GUSD_ADDED * TOKEN_PRICE + 1, 0, DEADLINE, transact={'from': user2}))
    ...
    assert contract.balanceOf(user1) == NEW_USER_ONE_BALANCE
    new_owner_fee = int(NEW_USER_ONE_BALANCE * 0.001)
    amount_to_transfer = int(NEW_USER_ONE_BALANCE * 0.997 * new_pool_size / new_total_supply / TOKEN_PRICE / 10**(18-2))
    contract.removeLiquidity(GUSD_token.address, NEW_USER_ONE_BALANCE, amount_to_transfer, DEADLINE, transact={'from': user1})
    owner_fee += new_owner_fee
    gusd_balance -= amount_to_transfer

    getcontext().prec = 16
    pool_size_change = int(Decimal(amount_to_transfer) * Decimal(TOKEN_PRICE) * Decimal(10**(18-2)))
    new_pool_size = int(Decimal(new_pool_size) - Decimal(pool_size_change))
    ...
    assert contract.balanceOf(user2) == 0
    assert contract.balanceOf(owner) == owner_fee
    assert GUSD_token.balanceOf(user1) == amount_to_transfer
    assert GUSD_token.balanceOf(contract.address) == gusd_balance

    # owner removes remaining liquidity
    dai_balance = DAI_token.balanceOf(contract.address)
    assert dai_balance == DAI_ADDED * 0.003 # 0.03
    new_total_liquidity = owner_fee
    getcontext().prec = 17
    amount_to_remove = int(Decimal(DAI_ADDED) * Decimal('0.003') * Decimal(TOKEN_PRICE) * Decimal(new_total_liquidity) / Decimal(new_pool_size))
    contract.removeLiquidity(DAI_token.address, amount_to_remove, 1, DEADLINE, transact={'from': owner})
    assert DAI_token.balanceOf(contract.address) == 0
    amount_to_remove = new_total_liquidity - amount_to_remove
...
def test_fees_with_gusd(w3, contract, DAI_token, GUSD_token, price_oracle, assert_fail):
    owner = w3.eth.accounts[0]
    user_dai = w3.eth.accounts[1]
    ...
    assert_fail(lambda: contract.removeLiquidity(GUSD_token.address, 1, 1, DEADLINE, transact={'from': user_dai}))

    # one user removes 0.02 GUSD
    gusd_to_remove = Decimal('0.02')
    amount_to_remove = ceil(gusd_to_remove * 10**18 * TOKEN_PRICE)
    owner_fee = int(amount_to_remove * 0.00001)
    gusd_received = int(gusd_to_remove * 10**2 * (Decimal(1) - TOTAL_FEE_PERCENTAGE)) # 0.01
    new_total_supply = TOTAL_SUPPLY_BEFORE - amount_to_remove + owner_fee
