...
def test_swap_tokens(w3, contract, oraclize, DAI_token, USDC_token, assert_fail):
    ...
    contract.updateOraclizeAddress(w3.eth.defaultAccount, transact={'from': w3.eth.defaultAccount})
    assert_fail(lambda: contract.__callback(contract.lastQueryId(), '1000000', transact={'from': w3.eth.defaultAccount}))
    ...
    contract.__callback(contract.lastQueryId(), '1000000', transact={'from': w3.eth.defaultAccount})
    ...
    contract.updateOraclizeAddress(oraclize.address, transact={'from': w3.eth.defaultAccount})
    ...
    contract.swapTokens(DAI_token.address, USDC_token.address, INPUT_AMOUNT, PRICE_LIMIT - 1, DEADLINE, transact={'from': user_address})
    contract.updateOraclizeAddress(w3.eth.defaultAccount, transact={'from': w3.eth.defaultAccount})
    assert_fail(lambda: contract.__callback(contract.lastQueryId(), '1000000', transact={'from': w3.eth.defaultAccount}))
    contract.updateOraclizeAddress(oraclize.address, transact={'from': w3.eth.defaultAccount})
