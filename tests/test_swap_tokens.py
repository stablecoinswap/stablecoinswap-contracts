...
def test_swap_with_gusd(w3, contract, price_oracle, DAI_token, GUSD_token, assert_fail):
    owner = w3.eth.defaultAccount
    user_address = w3.eth.accounts[1]
    ...
    # pool_size_after_swap = GUSD_ADDED * 10**(18-2) + (SWAP_FEE - owner_fee) * GUSD_TOKEN_PRICE
    owner_shares = 1048898656410768 # total_supply_before * owner_fee / pool_size_after_swap
    assert contract.balanceOf(owner) == total_supply_before + owner_shares #  ~2.001049 STL
    assert contract.totalSupply() == total_supply_before + owner_shares
